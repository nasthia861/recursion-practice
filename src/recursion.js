...
var factorial = function(n) {
 //if n more than or equal to zero, 
  if(n >= 0){
    //if n = 0, return 1 //Base Case
    if(n === 0){
      return 1;
    }
      // if n > 0, n * recursive action(n-1)
...
var sum = function(array) {
    ...
    //recursive action
    //add the first index in array + the function repeats itself till its empty
...
var reverse = function(string, outcome='') {
  //base case
  if(string.length === 0){
    return outcome;
  }
  //recursion
  outcome += string[string.length - 1];
  return reverse(string.slice(0, string.length - 1), outcome);
};

// 10. Write a function that determines if a string is a palindrome.
var palindrome = function(string) {
  //create new string to remove problems with spaces and cases
  let newString = string.toLowerCase().replaceAll(/ /g, '');
  //base case
  //if the string is 1 or less, return true
  if(newString.length <= 1){
    return true;
  }
  //recursion
  //if the first letter and the last letter match, return recursion with the ends split off
  if(newString[0] === newString[newString.length - 1]){
    return palindrome(newString.slice(1, newString.length - 1));
  }
  //if the string stays larger than one, return false
  return false;
};
...
var nthFibo = function(n, sequence = [0, 1]) {
  //base case
    ...
    if(n < 0){return null;}
    //first 2 index on fibonacci equal the index value
    if (n === 0 || n === 1) {
      return sequence [n];
    } 
    //if the sequence array is large enough to house the nth index, return the value of that index
    if(sequence[n]){
      return sequence[n]; 
    }
  //recursion
  //each recursion will add the next value to the sequence array, which is the sum of the last 2 values.
  sequence.push(sequence[sequence.length - 1] + sequence[sequence.length - 2])
  //function will call on itself till it passes sequence[n].
...
// var numToText = function(str, strArray=[], output=[]) {
//   //base
//   if(str.length === 0){
//     return output;
//   }
//   //recursion
//   strArray = str.split(' ');
//   if(strArray[0].typeOf !== 'number'){
//     output.push(strArray[0])
//   } else {output.push(toWords(strArray[0]))}
//   return numToText(str.split(' ').slice(1), strArray, output);
// };
...
//     if(str.includes(key)){
//       newStr.concat(str.replace(key, dictionary[key]))
//       console.log(newStr);
//     }  
//     return numToText(str.slice(str.indexOf(key), newStr));
//   }
// }
